...
func _draw() -> void:
	...
	for key in dungeon.visited_rooms.keys():
		var pos := Vector2(key) * CELL + Vector2(INSET, INSET)   # ← cast to Vector2
		draw_rect(Rect2(pos, inner_size), ROOM_CLR)

	# 3 · current player room (red inset square)
	var player_pos_px := Vector2(player_pos) * CELL + Vector2(INSET, INSET)
	draw_rect(Rect2(player_pos_px, inner_size), PLAYER_CLR)
